def get_the_input():
	...
	while(1):
		...
		each_job.append(int(input("Enter the priority")))
		each_job.append(0)
...
def update_ready_queue(jobs):
	...
	for each_job in jobs:
		if not_included(each_job) and (each_job[1] <= current_tick) and (each_job[4] < each_job[2]):
...
def select_job(ready_queue):
	...
	for each_job in ready_queue:
		if(each_job[4] < each_job[2]):
			selected_job = each_job
			break
	if(selected_job != []):
		for each_job in ready_queue:
			if(each_job[2] > each_job[4]):
				if(each_job[3] >  selected_job[3]):
					selected_job = each_job
		current_tick += selected_job[2] - 1
                selected_job[4] = selected_job[2]
