...
def select_job(ready_queue):
	...
	for each_job in ready_queue:
		if each_job[2] - each_job[3]  > time_quantum:
			each_job[3] += time_quantum
			current_tick += time_quantum - 1
			return each_job
		else:
			current_tick += each_job[2] - each_job[3] - 1
			each_job[3] = each_job[2]
			turn_around_time.append(list((each_job[0], current_tick - each_job[1])))
...
current_tick = 0
gant_chart = []
time_quantum = int(input("ENter the time quatum"))
